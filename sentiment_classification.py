...
Original file is located at
    https://colab.research.google.com/drive/1Zm3-PsbJrjE1RaXWPC0bhmxG4dCwr4m_
"""
...
# Step 8: Plot Confusion Matrix for the best model
...
# Step 9: Bar plot of Precision, Recall, F1-score for each model
def plot_metrics(models, X_train, y_train, X_val, y_val):
    ...
    # Convert to numpy array for bar plot
    x = np.arange(len(model_names))  # Label locations

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(x - 0.25, metrics['Precision'], width=0.25, label='Precision', color='lightblue')
    ax.bar(x, metrics['Recall'], width=0.25, label='Recall', color='lightgreen')
    ax.bar(x + 0.25, metrics['F1-Score'], width=0.25, label='F1-Score', color='salmon')

    # Add labels and title
    ax.set_xticks(x)
    ax.set_xticklabels(metrics['Model'])
    ax.set_ylabel('Score')
    ax.set_title('Precision, Recall, and F1-Score for Models')
    ax.legend()
...
plot_metrics(models, X_train, train_labels, X_val, val_labels)